/*
 * Taken from https://github.com/75lb/byte-size/blob/master/index.js
 */

type Unit = "metric" | "metric_octet" | "iec" | "iec_octet";
  ...
  constructor(bytes: number, options?: ByteSizeOptions) {
    ...
    if (table) {
      ...
      if (units) {
        ...
        const value =
          units.from === 0 ? prefix + bytes : prefix + defaultFormat.format(bytes / units.from);
 ...
 */
export function byteSize(bytes: number, options?: ByteSizeOptions): ByteSizeResult {
 ...
 */
byteSize.defaultOptions = function (options: ByteSizeOptions) {
